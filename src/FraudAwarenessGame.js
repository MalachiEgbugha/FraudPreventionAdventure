  ...
  const loadLeaderboard = () => {
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        // 2. Check if the key starts with 'game:'
        if (key && key.startsWith('game:')) {
          try {
            // 3. Get the value (which is a JSON string)
            const gameDataString = localStorage.getItem(key);
            
            if (gameDataString) {
              // The original code expected gameData.value, 
              // but localStorage.getItem returns the value directly.
              // Assuming gameDataString is the JSON string of the game object.
              const parsed = JSON.parse(gameDataString);
              allGames.push(parsed);
            }
          } catch (error) {
            console.error(`Error parsing game data for key ${key}:`, error);
          }
        }
      }
      ...
      // Since localStorage is fast (synchronous), you can remove the delay if needed.
      // We keep it inside finally for consistency with the original structure.
      setLoading(false);
    }
  };
...
const FraudAwarenessGame = () => {
  ...
  const saveGameResults = async () => {
    try {
      ...
      localStorage.setItem(resultKey, JSON.stringify(gameResults));
